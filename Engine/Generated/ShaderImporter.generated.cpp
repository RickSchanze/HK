...
static void Register_FShaderIntermediate_Impl()
{
    // 注册类型
    FTypeMutable Type = FTypeManager::Register<FShaderIntermediate>("ShaderIntermediate");

    // 注册属性
    FShaderIntermediate::Register_FShaderIntermediate_Properties(Type);

}

void FShaderIntermediate::Z_ShaderIntermediate_Register::Register_FShaderIntermediate()
{
    // 只注册类型注册器函数，不执行注册操作
    FTypeManager::RegisterTypeRegisterer<FShaderIntermediate>(Register_FShaderIntermediate_Impl);
}
