...
public:
    ...
    T* GetConfig()
    {
        ...
        // 先尝试通过名字查找
        IConfig** Found = Configs.Find(ConfigName);
        ...
        // 尝试从文件加载
        FString ConfigPath = GetConfigPath<T>();
        if (!ConfigPath.IsEmpty())
        {
            if (LoadConfigFromFile(NewConfig, ConfigPath))
            {
                HK_LOG_INFO(ELogcat::Config, "Loaded config from file: {}", ConfigPath.GetStdString());
            }
            else
            {
                HK_LOG_INFO(ELogcat::Config, "Config file not found, using default: {}", ConfigPath.GetStdString());
            }
        }
...
private:
    ...
    static bool LoadConfigFromFile(T* InConfig, const FString& InFilePath)
    {
        ...
        const std::filesystem::path FilePath(InFilePath.GetStdString());
        ...
        {
            HK_LOG_ERROR(ELogcat::Config, "Failed to load config from file {}: {}", InFilePath.GetStdString(), e.what());
            return false;
        }
    }
