...
// Concept: 检测是否存在 SavePrimitive() 且返回值不是 void
template <typename T>
concept CHasSavePrimitive = requires(T t) {
    { t.SavePrimitive() } -> CNotVoid;
};

// 辅助别名：获取 SavePrimitive 的返回类型 (去除引用和 const)
// 例如：如果 SavePrimitive 返回 const std::string&，这个就是 std::string
template <typename T>
using TPrimitiveType = std::decay_t<decltype(std::declval<T>().SavePrimitive())>;

// Concept: 检测是否存在 ReadPrimitive(Value)
// 强制要求 ReadPrimitive 接受的参数类型与 SavePrimitive 返回的类型一致
...
namespace cereal
{
    ...
    template <class Archive, class T>
        requires CHasSavePrimitive<T>
    auto save_minimal(const Archive&, const T& t) -> TPrimitiveType<T>
    {
        return const_cast<T&>(t).SavePrimitive();
    }

    // 适配 load_minimal
    // 关键点：直接使用 TPrimitiveType<T> 作为参数类型，而不是泛型 U
    template <class Archive, class T>
        requires CHasSavePrimitive<T> && CHasReadPrimitive<T, TPrimitiveType<T>>
